    ...
    return {
        ...
        "auto_memory_guard": (
            "BOOLEAN",
            {
                "default": True,
                "tooltip": (
                    "自动内存水位保护：按物理内存（RAM）水位动作，在每段开始前、二采前、"
                    "脸修前各检查一次。判定基准是「预测后水位 = 可用内存 − 上一段实测峰值 × 1.15」，"
                    "不是单纯的当前可用内存。三档：正常不动；偏低只清 CUDA 缓存、模型保留"
                    "（零磁盘写入）；危险才卸载模型（会触发模型重读与写盘，伤 SSD）。"
                    "阶段内另有 5% 硬地板，跌破只做 gc、绝不卸载。默认开。"
                ),
            },
        ),
        "memory_guard_low_percent": (
            "INT",
            {
                ...
                "step": 1,
                "tooltip": (
                    "偏低档水位（可用物理内存百分比）。低于此值只释放缓存、保留模型，"
                    "零磁盘写入，所以宁可早触发。默认 25%（32GB 内存 ≈ 8GB）。"
                    "【怎么调】日志里如果几乎全是危险档卸载，调大这个值没有用——因为判定用的是"
                    "「预测后水位」，预测为负时任何阈值都会落危险档；问题在单段峰值超出内存，"
                    "应减少每段帧数或分辨率。只有「想让缓存更早清掉」时才调大它。"
                ),
            },
        ),
        "memory_guard_critical_percent": (
            "INT",
            {
                ...
                "step": 1,
                "tooltip": (
                    "危险档水位（可用物理内存百分比）。低于此值才卸载模型。"
                    "默认 15%（32GB 内存 ≈ 4.8GB），给单段峰值留余量。"
                    "必须小于偏低档，否则偏低档失效（代码会自动钳制）。"
                    "【怎么调】跑崩了 → 调大（如 15→20），守卫更早卸载换安全；"
                    "想减少模型重读、少写盘 → 调小（如 15→8），代价是可能崩。"
                    "注意：若日志出现负数预测，说明单段峰值本身就超过可用内存，"
                    "此时把本值调到最小仍会触发危险档，调参无效，应降低单段开销。"
                ),
            },
        ),
    }
