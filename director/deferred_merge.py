...
# Max total uint8 video bytes allowed for the "decode everything, then concat"
# path. Above this the stream path is used instead (peak ≈ 1 segment + 24 frames).
PRECISE_MERGE_MAX_BYTES = 1_500_000_000
...
def _deferred_output_dir() -> Path:
    """Return the working directory under ComfyUI output for deferred merge.

    Centralizes all deferred-merge intermediates (slice pieces, wav files,
    concat list files, and final merged mp4) under
    ``<ComfyUI output>/minimax_seg_deferred/`` so they survive reboots and
    don't pollute ``%TEMP%`` (which Windows may silently purge, losing the
    final merged video). Falls back to system temp if output dir is
    unavailable.
    """
    try:
        import folder_paths  # type: ignore
        base = Path(folder_paths.get_output_directory()) / "minimax_seg_deferred"
    except Exception:
        base = Path(tempfile.gettempdir()) / "minimax_seg_deferred"
    try:
...
def _probe_mp4(path: Path) -> dict[str, Any]:
    ...
    # ffprobe may be unavailable (ComfyUI bundles only ffmpeg) or fail on some
    # files.  In that case fall back to imageio_ffmpeg's read_frames meta + an
    # ``ffmpeg -i`` stderr parse so we still get real width/height/fps/duration/
    # sample_rate instead of 0-frames probes that silently break the merge.
    if w <= 0 or h <= 0 or frames <= 0:
        meta = _imageio_probe_meta(path)
        if meta:
            sz = meta.get("size")
            if (w <= 0 or h <= 0) and sz and len(sz) == 2:
            ...
            if (duration_s <= 0) and mdur:
                try:
                    duration_s = float(mdur)
                except Exception:
                    pass
        if sr <= 0 or duration_s <= 0 or frames <= 0:
            extra = _fill_from_ffmpeg_stderr(path)
            if (w <= 0 or h <= 0):
                w = int(extra.get("width") or w)
                h = int(extra.get("height") or h)
            if duration_s <= 0:
                duration_s = float(extra.get("duration_s") or duration_s)
            if sr <= 0:
                sr = int(extra.get("sample_rate") or sr)
...
def _imageio_probe_meta(path: Path) -> dict[str, Any] | None:
    """Read only the metadata dict from imageio_ffmpeg without decoding frames.

    ``imageio_ffmpeg.read_frames`` yields the meta dict (codec / fps / size /
    duration) as its first item before any frame arrays, so this never decodes
    the whole clip.  Returns only the first meta dict or ``None``.
    """
    try:
        import imageio_ffmpeg  # type: ignore
        ...
        log.warning("deferred_merge: imageio meta probe failed on %s: %s", path, exc)
    return None


def _fill_from_ffmpeg_stderr(path: Path) -> dict[str, Any]:
    """Fall back to parsing ``ffmpeg -i`` stderr for duration / resolution / sample_rate.

    ffmpeg always prints human-readable stream info to stderr even without
    ffprobe being installed. Returns a dict with any discovered
    ``duration_s`` / ``width`` / ``height`` / ``sample_rate``.
    """
    out: dict[str, Any] = {}
    ffmpeg = _ffmpeg_bin()
    if not ffmpeg:
    ...
    if m:
        try:
            ...
            pass
    m = re.search(r"Audio:.*?\b(\d{3,6})\s*Hz", text)
    if m:
        try:
...
def _concat_copy(piece_paths: list[Path], out_path: Path) -> None:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="mmx_concat_", dir=str(_deferred_output_dir())))
...
def deferred_merge_with_seam_reencode(
    *,
    ...
    # Dispatch on BYTES, not frame count: the precise path decodes every segment
    # into CPU uint8 and then copies the slices again, so its peak is ~1–2× the
    # whole movie. A frame-count threshold OOMs on 1080p (4000 × 1920×1080×3
    # = 24.9 GB). Cap the precise path at a safe budget; stream everything else.
    pw = max((int(m["width"] or 0) for m in probes), default=0)
    ...
    if total_bytes <= PRECISE_MERGE_MAX_BYTES:
        log.info(
            "deferred_merge: 总帧数 %d (%.2f GB uint8) ≤ 上限，走 精确uint8拼接单文件编码 模式（零帧误差+零PTS跳变）",
            total_frames_sum, total_bytes / 1e9,
        )
    ...
    tmp_root = _deferred_output_dir() / f"_seam_merge_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        bat_path = _deferred_output_dir() / f"merge_{datetime.now().strftime('%Y%m%d_%H%M%S')}.bat"
        try:
        ...
        # tmp_root 的同级目录（_deferred_output_dir()），删除 tmp_root 不影响 out_path。
...
def _write_merged_mp4_uint8(path: Path, frames_u8: torch.Tensor, audio_wav: torch.Tensor, fps: float, sr: int) -> None:
    ...
    tmp_dir = _tmp.mkdtemp(prefix="mmx_merge_", dir=str(_deferred_output_dir()))
...
def _scheme_a_stream_merge(
    ...
    *,
    ...
    import tempfile as _tmp, wave as _wave
    tmp_dir = _tmp.mkdtemp(prefix="mmx_stream_", dir=str(out_dir))
