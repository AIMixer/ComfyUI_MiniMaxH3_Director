...
def pixel_anchor_latent(
    ...
    *,
    ...
    ``x0_low`` MUST be VAE-space (``latent_format.process_out`` already
    applied) — the caller in sample.py does this explicitly; feeding a raw
    model/callback-space x0 decodes unscaled latents.
    """
    if vae is None:
        return None
    video = x0_low
...
def lift_av_x0(
    ...
    *,
    ...
    NOTE: currently unused. If this gets wired up, ``av_x0`` arrives in
    model/callback space — apply ``latent_format.process_out`` to the video
    stream before the rho pixel-anchor path (see sample.py's inline path),
    otherwise ``pixel_anchor_latent`` decodes unscaled latents.
    """
