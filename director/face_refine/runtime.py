...
def apply_segment_face_refine(
    *,
    ...
    # Official H3 conditioning returns positive only. cfg≠1 selects CFGGuider,
    # which crashes/misbehaves on an empty negative list — encode an empty
    # prompt as the negative branch (fallback: force cfg=1 → BasicGuider).
    negative: list = []
    cfg_eff = float(cfg)
    if abs(cfg_eff - 1.0) >= 1e-6:
        try:
            neg_tokens = clip.tokenize("")
            negative = [[clip.encode_from_tokens_scheduled(neg_tokens), {}]]
        except Exception as exc:
            log.warning("FaceRefine: empty negative encode failed (%s); falling back to cfg=1.0.", exc)
            cfg_eff = 1.0
            negative = []
