...
def stitch_faces(
    ...
    # ``or <default>`` would resurrect user-set 0 values (the pack layer clamps
    # but keeps 0); only fall back when the key is absent/None.
    _dilation = pack.get("mask_dilation")
    dilation = 16 if _dilation is None else int(_dilation)
    _feather = pack.get("feather")
    feather = 24 if _feather is None else int(_feather)
    _colour_match = pack.get("colour_match")
    colour_match = 1.0 if _colour_match is None else float(_colour_match)
    _blend = pack.get("blend")
    blend = 1.0 if _blend is None else float(_blend)
