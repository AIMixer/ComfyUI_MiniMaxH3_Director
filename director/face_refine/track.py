...
def track_and_crop(
    ...
    for i, box in enumerate(boxes):
        ...
        sx = float(canvas_w) / bw
        sy = float(canvas_h) / bh
        # The crop box clamps at frame edges (above), so the face is NOT
        # always canvas-centered — derive the rect from the actual box,
        # otherwise the face_only/ellipse stitch mask slides off the face.
        fx = (float(sm_cx[i]) - box[0]) * sx - 0.5 * float(sm_fw[i]) * sx
        fy = (float(sm_cy[i]) - box[1]) * sy - 0.5 * float(sm_sz[i]) * sy
        face_rect.append(
            (
                fx,
                fy,
