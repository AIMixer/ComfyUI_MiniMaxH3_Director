...
def _segment_frame_counts_for_audio(
    ...
    *,
    ...
    if ui_counts:
        ...
        # 回退计数是未裁切的 UI 段长，而 total_frames 是 pin 相位对齐裁尾后的
        # 真实导出帧数（两者相差数帧）——直接按 UI 段长拼接会让合并音频超过
        # 视频总长，最终在时间轴末尾被硬砍掉数帧（段尾语音截断），同时每个
        # 接缝的音画偏移逐段累积。无逐缝裁切信息时按比例重标定：合并音频与
        # 视频严格等长（消除尾部硬砍），接缝偏移从累积的 5/10 帧级压到 ~1 帧级。
        s = sum(out)
        if s > 0 and int(total_frames) > 0 and s != int(total_frames):
            scaled = [max(0, int(round(c * total_frames / s))) for c in out]
            delta = int(total_frames) - sum(scaled)
            if delta != 0:
                # 舍入残差从尾部向前摊，保证合计严格等于 total_frames。
                for j in range(len(scaled) - 1, -1, -1):
                    ...
                    delta -= step
            log.info(
                "Audio fallback counts rescaled %s -> %s to match export total %d frames.",
                out, scaled, int(total_frames),
            )
            out = scaled
        return out
