...
def _tiled_model_call(
    ...
    **kwargs,
    ...
    if device.type == "cuda":
        # Once per tiled forward, not per tile: empty_cache() forces a device
        # sync and frees cached blocks that torch would otherwise reuse —
        # calling it inside the tile loop measurably slows tiled sampling.
