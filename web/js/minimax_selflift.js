...
function restoreConvertedWidget(node, name) {
    ...
    // Respect a user-wired input: converting it back to a widget would drop
    // the link without any prompt.
...
function ensureIntWidget(node, name, value, options, afterName) {
    ...
    if (w) {
        ...
        // Keep "converted-widget" as-is: rewriting it to "number" detaches the
        // widget from its input slot bookkeeping on the new frontend.
        if (!w.type) w.type = "number";
...
function fitSelfLiftNode(node) {
    ...
    contentH += 20;
    // Current litegraph signature is computeSize(out?: Size) — passing a
    // number throws (strict mode) and gets swallowed upstream, leaving this
    // fit a no-op. Call without args and read the computed height.
    const computed = Number(node.computeSize?.()?.[1]) || 0;
