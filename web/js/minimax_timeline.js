...
class MiniMaxH3DirectorEditor {
    ...
    syncExportPreFaceRefineUI() {
        this.timeline.output = this.timeline.output || {};
        // FaceRefine only applies to video tasks — hide the checkbox in image
        // batch mode where it can never run (otherwise it's dead UI noise).
        const show = !this.isImageBatch?.();
        this.exportPreFaceRefineWrap?.classList.toggle("hidden", !show);
        const w = this.widget("export_pre_face_refine");
        if (this.timeline.output.exportPreFaceRefine == null && w?.value) {
            this.timeline.output.exportPreFaceRefine = true;
        }
        const on = show && !!this.timeline.output.exportPreFaceRefine;
