...
def fit_edge_limit(
    ...
    *,
    ...
    if current <= limit:
        # Return the input untouched: early exits must preserve the caller's
        # rank (3D stays 3D) and keep the alpha channel, matching the
        # no-op contract of the 4D path.
        return image
    ...
    if image.shape[-1] > 3:
        # Preserve alpha on the scale path too (early exits keep it).
        alpha = image[..., 3:]
        if alpha.ndim == 3:
            alpha = alpha.unsqueeze(0)
        alpha_up = common_upscale(
            alpha.movedim(-1, 1), new_w, new_h, "area", "disabled"
        ).movedim(1, -1)
        out = torch.cat([out, alpha_up], dim=-1)
