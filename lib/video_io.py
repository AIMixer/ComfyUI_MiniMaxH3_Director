    ...
    for src_idx in unique:
        ...
        if not ok or bgr is None:
            if fallback is not None:
                if not hold_past_eof:
                    if native_count > 0 and native_frame < native_count - 1:
                        # A failed seek/decode mid-stream is not EOF — skip
                        # this index instead of truncating the whole clip.
                        log.warning(
                            "Failed to read frame %d (t=%.3fs) from %s; skipping.",
                            native_frame,
                            t_sec,
                            path,
                        )
